...
# path to combined iqtree output directory (will be created by script if needed)
CombineOUT=$OUTPUT/02.04_combined_iqtree_output
...
cat ${OUTPUT}/array_list.txt | while read array_list_line
do
  # read scf output for each locus
  cat ${OUTPUT}/${array_list_line} | while read aligned_loci_list_line
  do
